...
/** Compute the stub point one EXIT_STUB away from an anchor in the exit direction. */
function stubPoint(anchor: Pt, side: Side): Pt {
  switch (side) {
    case "top":    return { x: anchor.x, y: anchor.y - EXIT_STUB };
    case "bottom": return { x: anchor.x, y: anchor.y + EXIT_STUB };
    case "left":   return { x: anchor.x - EXIT_STUB, y: anchor.y };
    case "right":  return { x: anchor.x + EXIT_STUB, y: anchor.y };
  }
}
